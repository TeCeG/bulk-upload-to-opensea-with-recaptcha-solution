...
root.title("NFTs Upload to OpenSea v1.1.0")
...
class WebImage:
    ...
    def get(self):
        return self.image
imageurl = "https://uploads-ssl.webflow.com/6213cd0b4675a89606a7de55/622734486f97caac6e6f5cc2_banner2.png"
...
def main_program_loop():
    ...
    def solve_recaptcha():
        ...
        if displayOk:
            ...
            try:
                ...
                print("[ERR] Else, if audio recaptcha not available, Please solve recaptcha manually.")
    ...
    while end_num >= start_num:
        ...
        #manual solve recaptcha - Start-->
        time.sleep(2.5)
        #recaptcha completion check every 5 seconds
        count_a = 1
        create_1 = (f"/html/body/div[1]/div/main/div/div/section/div[2]/form/div[9]/div[1]/span/button")
        login_btn_xpath = create_1
        while True:
            if count_a == 1:
                print('-----------------------------------')
                print('|--> Human to Solve recaptcha. <--|')
                print('-----------------------------------')
            time.sleep(5)
            if check_exists_by_xpath(login_btn_xpath) is False:
                break
            if count_a == 120: #10 minutes
                messagebox.showwarning("Error!", "recaptcha Not solve!!")
                sys.exit("[ERR] recaptcha Not solve!!") 
            count_a = count_a + 1
        time.sleep(5)   
        #manual solve recaptcha - End-->
        ...
        try:
            ...
            try:
                time.sleep(1.5)
                cross = driver.find_element(By.XPATH, '/html/body/div[7]/div/div/div/div[2]/button/i')
                cross.click()
                time.sleep(0.8)
            except:
